...
function setupTween()
{
    ...
    if( script.api.tween.length == 0) {
        print( "Tween Alpha: There are no tweens found " + script.sceneObject.name + " SceneObject" );
        return;
    }
...
function setupAlphaComponentTweens( componentType, sceneObject )
{
   ...
   for (var i = 0; i < visualComponents.length; i++) {
        ...
        if(visualComponent.getTypeName() == "Component.Text"){
            ...
            {
                textProperties = getTextProperties( visualComponent );
            } 
            
        } else if( visualComponent.getMaterialsCount() == 0 ){
             return;
        }
    ...
    if ( script.recursive )
    {
        ...
        {
            setupAlphaComponentTweens( componentType, sceneObject.getChild(i) );
        }
    }
}
...
function getTextProperties(visualComponent){
    return {
    // Adding properties to track starting outline and drop shadow colors
        textOutlineAlpha : visualComponent.outlineSettings.fill.color.a,
        textDropShadowAlpha : visualComponent.dropshadowSettings.fill.color.a,
        backgroundAlpha : visualComponent.backgroundSettings.fill.color.a,
    }
}
...
function updateText(visualComponent, value){
    ...
    outlineColor.a = textProperties.textOutlineAlpha;
    ...
    dropShadowColor.a = textProperties.textDropShadowAlpha;
    ...
    backgroundColor.a = textProperties.backgroundAlpha;
